...
<div className='base'>
    ...
    <dl>
        ...
        <dd>Pandas 2.0+ no longer silently drops columns that can’t be aggregated after a groupby. We must use `.get()` to select the column(s) we want before `.groupby(...).mean()` (or other aggregations) so that our code runs properly on current pandas. </dd>
...
<DataFrameComponent data={'{"columns":["ID","Color","Weight","Age","Is_Cat","Owner_Comment"],"index":["cat","dog","hamster"],"data":[[3,3,3,3,3,3],[3,3,3,3,3,3],[2,2,2,2,2,2]]}'} />
...
---
:::info See Also
[`count()`](<../Grouping/aggregation/count().md>) · [`mean()`](<../Grouping/aggregation/mean().md>) · [`sum()`](<../Grouping/aggregation/sum().md>) · [`df.assign()`](<../building-organizing/df.assign().md>)
:::
